...
from dev_library_cli.render import OutputMode, output_json, output_json_line
from dev_library_cli.server.constants import API_PORT, CONFIG_DIR, DEVLIBRARY_HOME, LOG_DIR
...
def start(
    ...
    from dev_library_cli.server.deps import all_installed, install_dependencies
    from dev_library_cli.server.orchestrator import Orchestrator, ServiceError
    from dev_library_cli.server.updater import check_for_update
...
def stop(
    ...
    """
    from dev_library_cli.server.orchestrator import Orchestrator
...
def status(
    ...
    """
    from dev_library_cli.server.constants import CLICKHOUSE_HTTP_PORT, POSTGRES_PORT, REDIS_PORT
    from dev_library_cli.server.orchestrator import Orchestrator
...
def install(
    ...
    """
    from dev_library_cli.server.deps import install_dependencies
...
def _server_upgrade(version: str | None, skip_backup: bool, dry_run: bool, force: bool) -> dict:
    ...
    try:
        # Backup
        if not skip_backup:
            from dev_library_cli.server.backup import create_backup
...
def _server_rollback(from_backup: str | None, force: bool) -> dict:
    ...
    from dev_library_cli.server.backup import BACKUPS_DIR, list_backups, restore_backup
    from dev_library_cli.upgrade_lock import UpgradeLockError, acquire_lock, release_lock
...
def server_versions(
    ...
    """
    from dev_library_cli import version_check
    from dev_library_cli.server.backup import list_backups
