...
def doctor(
    ...
    with _capture(output):
        ...
        try:
            from dev_library_cli.lockfile_reconcile import plan_lockfile_reconciliation
        ...
        if should_fix:
            ...
            from dev_library_cli.skill_installer import install_observal_skill
...
def _check_observal_skill_missing() -> list[str]:
    ...
    from dev_library_cli.skill_installer import missing_observal_skill_harnesses
    ...
    for event in ("UserPromptSubmit", "Stop"):
        ...
        for g in groups:
            for h in g.get("hooks", []):
                if "dev_library_cli.hooks.session_push" in h.get("command", ""):
    ...
    for af in agent_profiles:
        ...
        for _event, entries in hooks.items():
            ...
            for h in entries:
                ...
                if "dev_library_cli.hooks.session_push --harness kiro" in command:
                    has_session_push = True
                    break
...
def _check_goose(issues: list, warnings: list):
    ...
    from dev_library_cli.harness_specs.goose_hooks_spec import GOOSE_HOOK_EVENTS, build_hooks, hooks_file
    from dev_library_cli.shared.utils import resolve_goose_config_dir, resolve_goose_data_dir
...
def _cleanup_goose(dry_run: bool) -> bool:
    ...
    from dev_library_cli.harness_specs.goose_hooks_spec import hooks_file, plugin_dir
...
def _patch_claude_code(dry_run: bool) -> bool:
    ...
    optic.trace("dry_run={}", dry_run)
    from dev_library_cli import settings_reconciler
...
def _patch_cursor(dry_run: bool) -> bool:
    ...
    cmd = f"{sys.executable} -m dev_library_cli.hooks.session_push --harness cursor"
...
def _patch_antigravity(dry_run: bool) -> bool:
    ...
    from dev_library_cli.harness_specs.antigravity_hooks_spec import (
        _OBSERVAL_HOOK_NAME,
...
def _patch_copilot(dry_run: bool) -> bool:
    ...
    from dev_library_cli.harness_specs.copilot_hooks_spec import build_copilot_hooks, build_copilot_run_hook_ps1
...
def _patch_opencode(dry_run: bool) -> bool:
    ...
    from dev_library_cli.harness_specs.opencode_hooks_spec import get_plugin_source
...
def _patch_goose(dry_run: bool) -> bool:
    ...
    from dev_library_cli.harness_specs.goose_hooks_spec import (
        build_hooks,
