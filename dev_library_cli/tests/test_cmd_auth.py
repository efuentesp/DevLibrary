...
def _patch_post_login_hooks(stack: ExitStack) -> MagicMock:
    ...
    for name in helper_names:
        stack.enter_context(patch(f"dev_library_cli.cmd_auth.{name}"))
    return stack.enter_context(patch("dev_library_cli.cmd_auth.config.save"))
...
class TestAuthLogin:
    ...
    def test_login_with_credentials_saves_config(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.get", side_effect=_fake_get))
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.post", side_effect=_fake_post_login))
    ...
    def test_login_with_invalid_credentials_exits_nonzero(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.get", side_effect=_fake_get))
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.post", side_effect=fake_post_unauth))
            stack.enter_context(patch("dev_library_cli.cmd_auth._post_login_setup"))
            mock_save = stack.enter_context(patch("dev_library_cli.cmd_auth.config.save"))
    ...
    def test_login_when_server_unreachable_exits_nonzero(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.get", side_effect=fake_get_unreachable))
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.post"))
            mock_save = stack.enter_context(patch("dev_library_cli.cmd_auth.config.save"))
...
class TestAuthWhoami:
    ...
    def test_whoami_outputs_email_and_role(self) -> None:
        ...
        with patch("dev_library_cli.cmd_auth.client.get", return_value=user_data) as mock_get:
    ...
    def test_whoami_json_output(self) -> None:
        ...
        with patch("dev_library_cli.cmd_auth.client.get", return_value=user_data):
...
class TestAuthStatus:
    ...
    def test_status_reports_ok_when_healthy(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.config.load", return_value=cfg),
            patch("dev_library_cli.cmd_auth.client.health", return_value=(True, 42.0)),
        ):
    ...
    def test_status_reports_unreachable_when_health_fails(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.config.load", return_value=cfg),
            patch("dev_library_cli.cmd_auth.client.health", return_value=(False, 0.0)),
        ):
            result = runner.invoke(app, ["auth", "status"])
...
class TestAuthJsonOutputs:
    def test_login_json_is_safe_and_skips_setup(self) -> None:
        with ExitStack() as stack:
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.get", side_effect=_fake_get))
            stack.enter_context(patch("dev_library_cli.cmd_auth.httpx.post", side_effect=_fake_post_login))
            stack.enter_context(patch("dev_library_cli.cmd_auth.config.load", return_value={}))
            save = stack.enter_context(patch("dev_library_cli.cmd_auth.config.save"))
            setup = stack.enter_context(patch("dev_library_cli.cmd_auth._post_login_setup"))
            banner = stack.enter_context(patch("dev_library_cli.cmd_auth.welcome_banner"))
            spinner = stack.enter_context(patch("dev_library_cli.cmd_auth.spinner"))
    ...
    def test_login_json_sso_preserves_stream_events_on_failure(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.httpx.get", side_effect=_fake_get),
            patch("dev_library_cli.cmd_auth.config.load", return_value={}),
            patch("dev_library_cli.cmd_auth._ensure_cli_matches_server"),
            patch("dev_library_cli.cmd_auth._do_device_flow_login", side_effect=device_flow),
        ):
            result = runner.invoke(
                app,
    ...
    def test_login_json_rejects_incomplete_credentials_without_prompt(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.httpx.get", side_effect=_fake_get),
            patch("dev_library_cli.cmd_auth.config.load", return_value={}),
            patch("dev_library_cli.cmd_auth.text_input", prompt),
            patch("dev_library_cli.cmd_auth.password_input", prompt),
        ):
            result = runner.invoke(
                app,
    ...
    def test_logout_json_reports_remote_and_local_results(self, tmp_path) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.config.CONFIG_FILE", config_file),
            patch("dev_library_cli.cmd_auth.config.CONFIG_DIR", tmp_path),
            patch("dev_library_cli.config.CONFIG_FILE", config_file),
            patch("dev_library_cli.config.CONFIG_DIR", tmp_path),
            patch("dev_library_cli.cmd_auth.httpx.post", return_value=response),
        ):
    ...
    def test_status_json_is_structured(self) -> None:
        ...
        with (
            patch.dict(sys.modules, {"dev_library_cli.telemetry_buffer": telemetry_buffer}),
            patch(
                "dev_library_cli.cmd_auth.config.load",
                return_value={"server_url": "http://localhost:8000", "access_token": "token"},
            ),
            patch("dev_library_cli.cmd_auth.client.health", return_value=(True, 42.5)),
        ):
    ...
    def test_status_json_failure_uses_stable_error(self) -> None:
        with patch("dev_library_cli.cmd_auth.config.load", return_value={"server_url": "http://localhost:8000"}):
            result = runner.invoke(app, ["auth", "status", "--output", "json"])
    ...
    def test_change_password_json_never_prompts(self) -> None:
        ...
        with (
            ...
            patch(
                "dev_library_cli.cmd_auth.client.put",
                return_value={"message": "Password changed"},
            ),
            patch("dev_library_cli.cmd_auth.password_input", password_prompt),
        ):
    ...
    def test_set_username_json_preserves_server_object(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.client.put", return_value=user),
            patch("dev_library_cli.cmd_auth.config.save") as save,
        ):
...
class TestConfigContracts:
    def test_show_json_uses_root_contract_without_secrets(self) -> None:
        ...
        with patch("dev_library_cli.cmd_auth.config.load", return_value=stored):
    ...
    def test_set_permission_failure_uses_permission_exit(self) -> None:
        ...
        with (
            patch("dev_library_cli.cmd_auth.config.save", side_effect=denied),
            patch("dev_library_cli.cmd_auth.config.load", return_value={"timeout": 30}),
        ):
    ...
    def test_aliases_empty_json_keeps_stable_shape(self) -> None:
        with patch("dev_library_cli.cmd_auth.config.load_aliases", return_value={}):
...
class TestPostLoginHarnessDetection:
    ...
    def test_opencode_detects_off_path_installer_binary(self, tmp_path, monkeypatch) -> None:
        ...
        from dev_library_cli.cmd_auth import _configure_opencode

        opencode_bin = tmp_path / ".opencode" / "bin" / "opencode"
        opencode_bin.parent.mkdir(parents=True)
        opencode_bin.write_text("")

        monkeypatch.setattr("dev_library_cli.cmd_auth.Path.home", lambda: tmp_path)
        monkeypatch.setattr("dev_library_cli.cmd_auth.shutil.which", lambda _name: None)

        with (
            patch("dev_library_cli.cmd_auth.typer.confirm", return_value=True),
            patch("dev_library_cli.cmd_auth._run_doctor_patch") as patch_doctor,
        ):
    ...
    def test_doctor_patch_uses_current_flags(self) -> None:
        from dev_library_cli.cmd_auth import _run_doctor_patch
