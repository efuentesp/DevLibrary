...
from dev_library_cli.errors import CliError


def test_anonymous_read_defaults_to_public_server():
    assert config.DEFAULTS["server_url"] == ""
    with patch("dev_library_cli.config.load", return_value=dict(config.DEFAULTS)):
...
def test_token_without_server_does_not_fall_back_to_public_instance():
    ...
    with patch("dev_library_cli.config.load", return_value=persisted), pytest.raises(CliError):
...
def test_optional_client_omits_authorization_header():
    ...
    with (
        patch("dev_library_cli.client.config.get_or_exit", return_value=cfg) as get_or_exit,
        patch("dev_library_cli.client._enforce_version_once"),
    ):
...
def test_optional_client_uses_token_when_available():
    ...
    with (
        patch("dev_library_cli.client.config.get_or_exit", return_value=cfg),
        patch("dev_library_cli.client._enforce_version_once"),
    ):
        token = client._OPTIONAL_AUTH.set(True)
        try:
...
def test_post_public_uses_optional_auth_client():
    ...
    with (
        patch("dev_library_cli.client._client", return_value=("https://public.observal.io", {})) as make_client,
        patch("dev_library_cli.client._request_with_retry", return_value=response),
    ):
