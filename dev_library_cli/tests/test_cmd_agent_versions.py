...
def test_agent_release_bumps_version(agent_yaml_dir: Path) -> None:
    ...
    with (
        patch("dev_library_cli.config.resolve_alias", return_value=agent_id),
        patch("dev_library_cli.client.get") as mock_get,
...
def test_agent_list_json_includes_pagination_metadata() -> None:
    ...
    with patch("dev_library_cli.cmd_agent.client.get_with_headers", return_value=(rows, {"x-total-count": "3"})):
...
def test_agent_pull_writes_rules_and_mcp(tmp_path: Path) -> None:
    ...
    with (
        patch("dev_library_cli.config.resolve_alias", return_value=agent_id),
        patch("dev_library_cli.client.get", return_value=agent_detail),
        patch("dev_library_cli.client.post_public", return_value=install_result),
    ):
        result = runner.invoke(
            app,
