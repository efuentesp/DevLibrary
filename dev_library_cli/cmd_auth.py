...
from dev_library_cli.branding import welcome_banner
from dev_library_cli.errors import CliError, ErrorCategory, fail
from dev_library_cli.prompts import password_input, quick_choice, text_input
from dev_library_cli.render import OutputMode, console, esc, kv_panel, output_json, spinner, status_badge
...
def _ensure_cli_matches_server(server_url: str) -> None:
    ...
    from dev_library_cli.version_check import get_current_version
    ...
    from dev_library_cli.install_detector import upgrade_command
...
def login(
    ...
    from dev_library_cli.lockfile import migrate_lockfile_v1
...
def register_config(app: typer.Typer):
    ...
    def config_set(
        ...
        if key == "server_url":
            from dev_library_cli.lockfile import LOCKFILE_PATH, migrate_lockfile_v1
...
def _post_login_setup():
    ...
    try:
        ...
        from dev_library_cli.cmd_doctor import doctor
...
def _post_auth_onboarding():
    ...
    try:
        ...
        for label, (dir_path, ide_key) in _ide_dirs.items():
            ...
            agents = mcps = 0
            try:
                from dev_library_cli.harness import NotSupportedError, ensure_loaded, get_adapter
...
def _generate_initial_layer_snapshot():
    ...
    """
    try:
        from dev_library_cli.layer import ensure_local_snapshot
...
def _run_doctor_patch(ide_name: str):
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "dev_library_cli.main", "doctor", "patch", "--harness", ide_name],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
