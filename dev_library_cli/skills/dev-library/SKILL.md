...
Use this skill for core account, setup, local inventory, inbox, and teamspace work. Use the specialized `dev-library-agents`, `dev-library-registry`, `dev-library-ops`, `dev-library-admin`, or `dev-library-advanced` skill when its description matches more closely.
...
| Create, edit, release, or pull an Agent | Use `dev-library-agents` |
| Search, submit, install, or version a component | Use `dev-library-registry` |
| Traces, telemetry, logs, ratings, or insight reports | Use `dev-library-ops` |
| Reviews, users, settings, security, or server administration | Use `dev-library-admin` |
| Reconciliation, CLI version recovery, or explicit offline fallback | Use `dev-library-advanced` |
...
| Unavailable or not configured | Stop and use `dev-library-advanced` only if the user still wants an explicit fallback |
