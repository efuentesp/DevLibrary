...
Session telemetry uses a Goose hook plugin. `dev-library doctor patch --harness
...
| Telemetry | Hook push plus `dev-library reconcile` |
...
```bash
dev-library agent pull <agent-name> --harness goose
```
...
```bash
dev-library agent pull <agent-name> --harness goose --scope project
```
...
```bash
dev-library doctor patch --harness goose
```
...
```bash
dev-library scan --harness goose
dev-library doctor
```
...
session reading resolve those locations automatically. `dev-library agent pull`
...
ever touches the entries it owns. `dev-library scan` additionally reports legacy
...
```bash
dev-library doctor cleanup --harness goose
```
...
```bash
dev-library reconcile --harness goose
dev-library reconcile --harness goose --since 720 --dry-run
```
...
event or by `dev-library reconcile`.
